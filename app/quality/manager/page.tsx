  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold">Quality Approval</h1>
          <p className="text-slate-500 mt-2">
            Review quality issues waiting for manager approval.{" "}
          </p>
        </div>

        {issues.length === 0 ? (
          <Card className="p-8 text-center text-slate-500">
            No quality issues waiting for approval.{" "}
          </Card>
        ) : (
          issues.map((issue) => (
            <Card
            ...
            >
              ...
              <Button asChild className="cursor-pointer">
                <Link href={`/quality/manager/${issue.id}`}>Open</Link>
              </Button>
            </Card>
          ))
        )}
